...
if($_POST['cmd'] == "edit")
{
    ...
    */
    ...
    $str = "UPDATE prospeccao 
        SET idusuario = '$adm_codigo', nome = '$nome', sobrenome = '$sobrenome', data_nascimento = '$data_nascimento', data_exame = '$data_exame', porque = '$porque', 
...
<div class="wrapper wrapper-content animated fadeInRight">
            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content" <?=($ind == 2) ? 'style="display: block;"' : 'style="display: none;"'?>>
                    <form method="post" class="form-horizontal" name="form" id="form" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="url" id="url" value="<?=($url) ? base64_decode($url) : ''?>">

                        <div class="row">
                            <div class="col-md-3">
                                <p class="font-bold">
                                    Nome<?=($perm_pesquisa_obrigatoriedade == 1) ? '*' : ''?>
                                </p>
                                <input type="text" name="nome" id="nome" class="form-control" value="<?=stripslashes($vet['nome'])?>" <?=($perm_pesquisa_obrigatoriedade == 1) ? 'required' : ''?>>
                            </div>
                            <div class="col-md-3">
                                <p class="font-bold">
                                    Sobrenome
                            ...
                            <div class="col-md-3">
                                <p class="font-bold">
                                    Data de nascimento<?=($perm_pesquisa_obrigatoriedade == 1) ? '*' : ''?>
                                </p>
                                <input type="text" name="data_nascimento" id="data_nascimento" class="form-control" value="<?=$vet['data_nascimento']?>" <?=($perm_pesquisa_obrigatoriedade == 1) ? 'required' : ''?>>
                            </div>
                            <div class="col-md-3">
                                <p class="font-bold">
                                    Último exame<?=($perm_pesquisa_obrigatoriedade == 1) ? '*' : ''?>
                                </p>
                                <input type="text" name="data_exame" id="data_exame" class="form-control" value="<?=$vet['data_exame']?>" <?=($perm_pesquisa_obrigatoriedade == 1) ? 'required' : ''?>>
                            </div>
                        </div>
                        <br>
                        <div class="row">
                            <div class="col-md-12">
                                <p class="font-bold">
                                    Porquê<?=($perm_pesquisa_obrigatoriedade == 1) ? '*' : ''?>
                                </p>
                                <input type="text" name="porque" id="porque" class="form-control" value="<?=stripslashes($vet['porque'])?>" <?=($perm_pesquisa_obrigatoriedade == 1) ? 'required' : ''?>>
                            </div>                            
                        </div>
                        <br>
                        <?
    ...
    if($chave)
    {
        ...
        $strWhere .= " AND (nome LIKE '%$chave%' OR sobrenome LIKE '%$chave%')";
    }
    ...
    $str = "SELECT * FROM prospeccao WHERE idempresa = '$adm_empresa' AND status IN ('1','2','3','7','8') AND auto = '0' $strWhere $strWherePesq ORDER BY nome";
    $rs  = mysqli_query($conexao, $str) or die(mysqli_error($conexao));
    $num = mysqli_num_rows($rs);
                            ...
                            <tbody>
                                ...
                                <tr class="gradeX">
                                    ...
                                    <td>
                                        ...
                                        Tel. 2: <?=($vet['telefone_2']) ? '<a href="https://api.whatsapp.com/send?phone=55'.preg_replace('/[^\d]/', '', $vet['telefone_2']).'" target="_blank">'.$vet['telefone_2'].'</a>' : 'Não informado'?>
                                    </td>
