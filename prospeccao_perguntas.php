...
if($_POST['cmd'] == "add")
{   
    $str = "INSERT INTO prospeccao_perguntas (pergunta, status) VALUES ('$pergunta', '$status')";
    $rs  = mysqli_query($conexao, $str) or die(mysqli_error($conexao));

    redireciona("prospeccao_perguntas.php?ind_msg=1");
}

if($_POST['cmd'] == "edit")
{
    $str = "UPDATE prospeccao_perguntas SET pergunta = '$pergunta', status = '$status' WHERE codigo = '$codigo'";
    $rs  = mysqli_query($conexao, $str) or die(mysqli_error($conexao));

    redireciona("prospeccao_perguntas.php?ind_msg=2");
}

if($_GET['cmd'] == "del")
{
    $str = "DELETE FROM prospeccao_perguntas WHERE codigo = '$codigo'";
    $rs  = mysqli_query($conexao, $str) or die(mysqli_error($conexao));
    
    redireciona("prospeccao_perguntas.php?ind_msg=3");
}
...
$str = "SELECT * FROM prospeccao_perguntas WHERE codigo = '$codigo'";
$rs  = mysqli_query($conexao, $str) or die(mysqli_error($conexao));
$vet = mysqli_fetch_array($rs);

$title = "'View Optica<br>Perguntas (prospecção)'";
$columns = '0, 1';
$order = ',order: [[ 0, "asc" ]]';
?>
...
<div class="wrapper wrapper-content animated fadeInRight">
    ...
    <?
    $str = "SELECT * FROM prospeccao_perguntas ORDER BY pergunta";
    $rs  = mysqli_query($conexao, $str) or die(mysqli_error($conexao));
                        ...
                        <table id="list" class="table table-striped table-bordered table-hover dataTables-example" >
                            ...
                            <tbody>
                                ...
                                <tr class="gradeX">
                                    ...
                                    <td><?=$status?></td>
                                    <td class="center">
                                        <a class="btn btn-warning btn-circle" type="button" title="editar / visualizar" href="prospeccao_perguntas.php?ind=2&codigo=<?=$vet['codigo']?>"><i class="fa fa-edit"></i></a>
                                        <a class="btn btn-danger btn-circle" type="button" title="excluir" href="prospeccao_perguntas.php?cmd=del&codigo=<?=$vet['codigo']?>" onclick="javascript: if(!confirm('Deseja realmente excluir este registro?')) { return false }"><i class="fa fa-trash-o"></i></a> 
                                    </td>
                                </tr>
                                <?
                                }
